...
function buddyforms_simple_auctions_add_wc_form_element_tab($form_fields,$field_type,$field_id){
	...
	$form_fields['Auctions']['_auction_item_condition']	= new Element_Select( '<b>' . __('Item Condition', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_auction_item_condition]", array( 'display' => __('Display', 'buddyforms'), 'new' => __('New', 'buddyforms'), 'used'=> __('Used', 'buddyforms')), array('value' => $auction_item_condition));

	// Auction Type
	$auction_type = isset($buddyform['form_fields'][$field_id]['_auction_type']) ? $buddyform['form_fields'][$field_id]['_auction_type'] : 'display';
	$form_fields['Auctions']['_auction_type']	= new Element_Select( '<b>' . __('Auction Type', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_auction_type]", array( 'display' => __('Display', 'buddyforms'), 'normal'  => __('Normal', 'buddyforms'), 'reverse' =>__('Reverse', 'buddyforms') ), array('value' => $auction_type));

	// Auction Proxy
	$auction_proxy = isset($buddyform['form_fields'][$field_id]['_auction_proxy']) ? $buddyform['form_fields'][$field_id]['_auction_proxy'] : 'display';
	$form_fields['Auctions']['_auction_proxy']	= new Element_Select( '<b>' . __('Auction Proxy', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_auction_proxy]", array( 'display' => __('Display', 'buddyforms'), 'enable'  => __('Enable', 'buddyforms'), 'disable' =>__('Disable', 'buddyforms') ), array('value' => $auction_proxy));
	...
	$form_fields['Auctions']['_auction_start_price']	= new Element_Select( '<b>' . __('Start Price', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_auction_start_price]", array( 'none' => __('None', 'buddyforms'), 'required'  => __('Required', 'buddyforms'), 'hidden' =>__('Hidden', 'buddyforms') ), array('value' => $start_price));
	...
	$form_fields['Auctions']['_auction_bid_increment']	= new Element_Select( '<b>' . __('Bid increment', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_auction_bid_increment]", array( 'none' => __('None', 'buddyforms'), 'required'  => __('Required', 'buddyforms'), 'hidden' =>__('Hidden', 'buddyforms') ), array('value' => $auction_bid_increment));
	...
	$form_fields['Auctions']['_auction_reserved_price']	= new Element_Select( '<b>' . __('Reserve price', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_auction_reserved_price]", array( 'none' => __('None', 'buddyforms'), 'required'  => __('Required', 'buddyforms'), 'hidden' =>__('Hidden', 'buddyforms') ), array('value' => $auction_reserved_price));

	// Buy it now price
	$regular_price = isset($buddyform['form_fields'][$field_id]['_regular_price']) ? $buddyform['form_fields'][$field_id]['_regular_price'] : 'none';
	$form_fields['Auctions']['_regular_price']	= new Element_Select( '<b>' . __('Buy it now price', 'buddyforms') . '</b>', "buddyforms_options[form_fields][".$field_id."][_regular_price]", array( 'none' => __('None', 'buddyforms'), 'required'  => __('Required', 'buddyforms'), 'hidden' =>__('Hidden', 'buddyforms') ), array('value' => $regular_price));
...
function buddyforms_product_write_panel($thepostid, $customfield){
	...
	if(isset($customfield['_auction_item_condition']) && $customfield['_auction_item_condition'] == 'display'){
		woocommerce_wp_select( 	array( 'custom_attributes' => $required, 'id' => '_auction_item_condition', 'label' => $required_html.__( 'Item condition', 'wc_simple_auctions' ), 'options' => apply_filters( 'simple_auction_item_condition',$auction_item_condition) ) );
	} else {
	...
	if(isset($customfield['_auction_type']) && $customfield['_auction_type'] == 'display'){
		woocommerce_wp_select( 	array( 'custom_attributes' => $required, 'id' => '_auction_type', 'label' => $required_html.__( 'Auction type', 'wc_simple_auctions' ), 'options' => $auction_types ) );
	} else {
	...
	if(isset($customfield['_auction_proxy']) && $customfield['_auction_proxy'] == 'display'){
		woocommerce_wp_checkbox( 	array( 'custom_attributes' => $required, 'id' => '_auction_proxy', 'label' => $required_html.__( 'Proxy bidding?', 'wc_simple_auctions' ), 'description' => __( 'Enable proxy bidding', 'wc_simple_auctions' ), 'cbvalue'=>'yes', 'checked'=>'true' ) );
	} else {
	...
	if(isset($customfield['_auction_start_price']) && $customfield['_auction_start_price'] != 'hidden'){
		...
		woocommerce_wp_text_input( array( 'custom_attributes' => $required,  'id' => '_auction_start_price', 'class' => 'wc_input_price short', 'label' => $required_html.__( 'Start Price', 'wc_simple_auctions' ) . ' ('.get_woocommerce_currency_symbol().')', 'type' => 'number', 'custom_attributes' => array(
			'step' 	=> 'any',
			'min'	=> '0'
		) ) );
	} else {
	...
	if(isset($customfield['_auction_bid_increment']) && $customfield['_auction_bid_increment'] != 'hidden'){
		...
		woocommerce_wp_text_input( array( 'custom_attributes' => $required,  'id' => '_auction_bid_increment', 'class' => 'wc_input_price short', 'label' => $required_html.__( 'Bid increment', 'wc_simple_auctions' ) . ' ('.get_woocommerce_currency_symbol().')', 'type' => 'number', 'custom_attributes' => array(
			'step' 	=> 'any',
			'min'	=> '0'
		) ) );
	} else {
	...
	if(isset($customfield['_auction_reserved_price']) && $customfield['_auction_reserved_price'] != 'hidden'){
		...
		woocommerce_wp_text_input( array( 'custom_attributes' => $required, 'id' => '_auction_reserved_price', 'class' => 'wc_input_price short', 'label' => $required_html. __( 'Reserve price', 'wc_simple_auctions' ) . ' ('.get_woocommerce_currency_symbol().')', 'type' => 'number', 'custom_attributes' => array(
			'step' 	=> 'any',
			'min'	=> '0'
		),'desc_tip' => 'true',
